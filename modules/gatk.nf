...
process genomicsDBImport {
	...
	--genomicsdb-workspace-path genomicsDB_${chr} \
	--batch-size 50 \
	--consolidate true \
	--tmp-dir tmp_${chr} \
	-L $chr \
...
process mutect2{
    ...
    output:
    path("unfiltered.vcf"), emit: unfiltered_vcf
    path("f1r2.tar.gz"), emit: f1r2
    path("unfiltered.vcf.stats"), emit:stats
...
process calculate_contamination{
    ...
    output:
    path("segments.table"), emit: segmentation_table
    path("calculatecontamination.table"), emit: contamination_table
